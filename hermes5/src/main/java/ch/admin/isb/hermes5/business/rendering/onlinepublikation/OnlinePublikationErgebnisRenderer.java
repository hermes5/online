...
public class OnlinePublikationErgebnisRenderer extends OnlinePublikationRenderer {
    ...
    public boolean isResponsibleFor(AbstractMethodenElement methodElement) {
        return methodElement instanceof Ergebnis;
    }
    ...
    public String renderModelElement(AbstractMethodenElement methodenElement, List<MenuItem> adjustedMenu,
        ...
        initContext.put("relationshipTableRecords", relationshipTablePreprocessor.preprocess(
                ((Ergebnis) methodenElement).getRelationshipTableRecords(), localizationEngine));

        initContext.put("WebFont", WebFont.class);

        return velocityAdapter.mergeTemplates(initContext, OP_HEADER_AND_MENU, "ergebnis.vm", OP_FOOTER);
    }
}
