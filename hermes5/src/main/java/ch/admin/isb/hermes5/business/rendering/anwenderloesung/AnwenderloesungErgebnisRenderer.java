...
public class AnwenderloesungErgebnisRenderer extends AnwenderloesungModelElementRenderer {
    ...
    public boolean isResponsible(AbstractMethodenElement modelElement) {
        return modelElement instanceof Ergebnis;
    }
    ...
    public String renderModelElement(AbstractMethodenElement modelElement, LocalizationEngine localizationEngine,
        ...
        ctx.put("templateprefix", "../../templates/"+localizationEngine.getLanguage()+"/");

        ctx.put("titleKey", "al_doc_type_ergebnis");

        ctx.put("relationshipTableRecords", relationshipTablePreprocessor.preprocess(
                ((Ergebnis) modelElement).getRelationshipTableRecords(), localizationEngine));

        ctx.put("WebFont", WebFont.class);

        return velocityAdapter.mergeTemplates(ctx, "al_header.vm", "ergebnis.vm", "al_footer.vm");
    }
}
